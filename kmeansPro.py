...
class kmeansPro:
    ...
    def visual_ploting(self,axis_plot=None,new_candiates=None,method=1,K_nn=3):
        ...
        for i in range(K):
            plt.scatter(data[idx == i, axis_plot[0]], data[idx == i, axis_plot[1]],s=80,color=colors[i],  label='Cluster ' + str(i+1))
            plt.scatter([centers[i,axis_plot[0]]], [centers[i,axis_plot[1]]],marker='*',s=80,color=colors[i],  label='Centers cluster'+str(i+1))
        ...
        if new_candiates is not None:
            ...
            for i in range(K):
                if np.sum(candidates_idx==i)>0:
                    plt.scatter([new_candiates[candidates_idx==i,axis_plot[0]]], [new_candiates[candidates_idx==i,axis_plot[1]]],marker='h',s=200,color=colors[i],  label='New Candidates of cluster '+str(i+1))
